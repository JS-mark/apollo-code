...
describe('validateWorkspacePath', () => {
  ...
  it.each([parse(process.cwd()).root, homedir()])('rejects protected root %s', async (path) => {
    await expect(validateWorkspacePath(path)).rejects.toMatchObject({ code: 'APOLLO_UNSAFE_CWD' })
  })
  ...
  it('rejects sensitive prefixes after resolving a symlink', async () => {
    ...
    fixtures.push(root)
    await mkdir(join(homedir(), '.apollo'), { recursive: true })
    await symlink(join(homedir(), '.apollo'), link)
    await expect(validateWorkspacePath(link)).rejects.toMatchObject({ code: 'APOLLO_UNSAFE_CWD' })
  })
})
