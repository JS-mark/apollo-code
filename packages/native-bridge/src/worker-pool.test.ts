...
function fakeWorker() {
  const child = new EventEmitter() as EventEmitter & { stdin: PassThrough; stdout: PassThrough; stderr: PassThrough; kill: ReturnType<typeof vi.fn>; pid: number }
  child.stdin = new PassThrough()
  child.stdout = new PassThrough()
  child.stderr = new PassThrough()
  child.kill = vi.fn()
  child.pid = 42
  queueMicrotask(() => child.stdout.write('{"jsonrpc":"2.0","method":"worker.ready","params":{"protocol":1}}\n'))
  return child
}
...
describe('WorkerPool', () => {
  ...
  it('degrades after three crashes in one session', async () => {
    ...
    const pool = new WorkerPool({ resolve: async () => '/worker', spawn: () => children.shift() as never })
  ...
  it('reaps an idle worker', async () => {
    vi.useFakeTimers()
    const child = fakeWorker()
    const pool = new WorkerPool({ idleMs: 30, resolve: async () => '/worker', spawn: () => child as never })
