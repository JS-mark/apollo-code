...
export class RpcPeer {
  ...
  constructor(input: Readable, private readonly output: Writable) {
    createInterface({ input }).on('line', (line) => {
      ...
      frame.error ? deferred.reject(new Error(frame.error.message ?? 'worker RPC failed')) : deferred.resolve(frame.result)
    })
  }

  request(method: string, params: unknown): Promise<unknown> { return this.requestWithId(this.nextId++, method, params) }

  notification(method: string): Promise<unknown> {
    return new Promise(resolve => this.notifications.set(method, [...this.notifications.get(method) ?? [], resolve]))
  }
