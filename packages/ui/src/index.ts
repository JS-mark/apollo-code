...
export interface ModelAlias { alias: string; model: string }
export interface PickerCandidate { kind: 'file' | 'model'; label: string; value: string }
...
export function applyPickerSelection(input: string, candidate: PickerCandidate): PickerSelection {
  ...
  return { attachment: candidate.value, text: `[@file:${candidate.value}]${suffix ? ` ${suffix}` : ''}` }
}

export interface PermissionPrompt { description: string; diff?: string; id: string; risk: 'high' | 'low' | 'medium' }
...
export function renderPermissionPrompt(prompt: PermissionPrompt): string {
  return [`Permission required [${prompt.risk.toUpperCase()}]`, prompt.description, prompt.diff ? renderDiff(prompt.diff) : '', '[allow once] [allow session] [deny]'].filter(Boolean).join('\n')
}
export function renderDiff(diff: string): string {
  return diff.split('\n').map(line => line.startsWith('+') && !line.startsWith('+++') ? `+ ${line.slice(1)}` : line.startsWith('-') && !line.startsWith('---') ? `- ${line.slice(1)}` : `  ${line}`).join('\n')
}
