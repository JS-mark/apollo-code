...
export function updateSession(state: SessionState, recipe: (draft: SessionState) => void): SessionState {
  return produce(state, (draft) => { recipe(draft as SessionState); draft.version += 1 })
}
