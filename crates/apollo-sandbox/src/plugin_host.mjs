...
const rpc = async (method, params) => {
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => { pending.delete(id); reject(new Error('bridge call timed out')) }, 10_000)
...
input.on('data', chunk => {
  ...
  for (;;) {
    ...
    if (!line) continue
    let message; try { message = JSON.parse(line) } catch { throw new Error('invalid bridge JSON') }
    if (message.jsonrpc !== '2.0' || message.bridgeVersion !== VERSION) throw new Error('unsupported bridge protocol')
    ...
    if (message.method === 'callback.invoke' && message.id != null) {
      ...
      }).then(result => write({ jsonrpc: '2.0', bridgeVersion: VERSION, id: message.id, result: encode(result) }), error => write({ jsonrpc: '2.0', bridgeVersion: VERSION, id: message.id, error: { code: -32000, message: String(error?.message || error) } }))
    }
  }
})
input.on('end', () => { for (const item of pending.values()) { clearTimeout(item.timeout); item.reject(new Error('bridge closed')) } process.exitCode = 1 })
const encode = value => {
  if (typeof value === 'function') { const callbackId = `callback-${nextId++}`; callbacks.set(callbackId, value); return { $callback: callbackId } }
  return Array.isArray(value) ? value.map(encode) : value && typeof value === 'object' ? Object.fromEntries(Object.entries(value).map(([k,v]) => [k, encode(v)])) : value
}
const proxy = path => new Proxy(() => {}, { get: (_, key) => key === 'then' ? undefined : proxy([...path, String(key)]), apply: (_, __, args) => rpc(`apollo.${path.join('.')}`, encode(args.length === 1 ? args[0] : args)) })
