...
afterEach(async () => Promise.all(fixtures.map(path => rm(path, { force: true, recursive: true }))))

function ports(overrides: Partial<ApolloPorts> = {}): ApolloPorts {
  return {
    version: '0.0.0-test',
    native: { probe: vi.fn(async () => ({ tier: 'full' as const, mechanism: 'test sandbox', features: { filesystem: true, network: true }, degradationReasons: [] })), health: vi.fn(async () => ({ sandbox: true, search: false, fs: false })) },
    auth: { health: vi.fn(async () => ({ configured: false, detail: 'anthropic credential unavailable' })) },
...
describe('runCli', () => {
  ...
  it('normalizes --cwd before starting a session', async () => {
    ...
    expect(testPorts.session.start).toHaveBeenCalledWith(expect.objectContaining({ cwd: nested, prompt: 'hello' }))
  })
  ...
  it('shows a red warning and records permission bypass once', async () => {
    ...
    expect(testPorts.telemetry.securityEvent).toHaveBeenCalledWith('permissions.dangerously_skipped', expect.any(Object))
  })

  it('exits 3 when strict sandbox receives a degraded tier', async () => {
    const testPorts = ports({ native: { probe: vi.fn(async () => ({ tier: 'partial' as const, mechanism: 'landlock', features: { filesystem: true, network: false }, degradationReasons: ['no seccomp'] })), health: vi.fn(async () => ({ sandbox: true, search: false, fs: false })) } })
  ...
  it('never enters a none-tier session without explicit confirmation', async () => {
    const testPorts = ports({ native: { probe: vi.fn(async () => ({ tier: 'none' as const, mechanism: 'unavailable', features: { filesystem: false, network: false }, degradationReasons: ['probe failed'] })), health: vi.fn(async () => ({ sandbox: false, search: false, fs: false })) } })
