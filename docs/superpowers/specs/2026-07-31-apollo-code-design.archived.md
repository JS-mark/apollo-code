...
│  ├─ plugin-runtime/             # Plugin 装载器（apollo-sandbox --run-plugin 子进程 + JSON-RPC bridge + manifest 权限）
...
│  │
│  └─ ── 基础 ──
│     └─ shared/                  # 类型/常量/logger/ApolloError/config schema (zod)
│
├─ crates/                        # Rust workspace
│  ├─ apollo-sandbox/             # 独立二进制：syscall 隔离执行器
│  ├─ apollo-search/              # napi-rs addon：ripgrep + tree-sitter
│  ├─ apollo-fs/                  # napi-rs addon：大文件 diff / tokenize
│  └─ xtask/                      # Rust 侧构建/发布脚本
│
...
```
  ...
  通过 tool-kit 注入工具（core 完全不感知）：
    mcp-client / plugin-runtime / memory-runtime(via apollo.memory bridge) / subagent(via Task tool)
...
- `auth → native-bridge` 是**运行时可选**依赖（不是 type-only）：仅 L4 加密凭据文件的 AES-256-GCM 操作走 Rust（`apollo-fs` 的 `encrypt/decrypt` 导出）。L1-L3 只用 Node `crypto`，`native-bridge` 缺失时 auth 自动 fallback。CI 允许该依赖，但 `auth` 内所有 native-bridge 调用必须包 try/catch fallback。
...
| **ApolloBridge / Plugin / ToolSpec 等 SDK 类型**         | **plugin-sdk**（发布 npm，作者依赖）            |
...
```
platforms/
  native-sandbox-<os>-<arch>/    → 打包 apollo-sandbox 可执行二进制
  native-search-<os>-<arch>/     → 打包 apollo-search.node napi 产物
  native-fs-<os>-<arch>/         → 打包 apollo-fs.node napi 产物
```
...
```json
{
  "optionalDependencies": {
    "@apollo-code/native-sandbox-darwin-arm64": "workspace:*",
    "@apollo-code/native-sandbox-darwin-x64": "workspace:*",
    "@apollo-code/native-sandbox-linux-x64-gnu": "workspace:*",
    ...(共 18 项)
  }
}
```

**运行时解析**：`native-bridge` 通过 `process.platform + process.arch` 拼出目标包名，`require.resolve` 找到实际文件路径。找不到就回落到纯 JS fallback（`apollo-search` fallback = fast-glob + JS 正则；`apollo-sandbox` fallback = 拒绝 + 提示 --dangerous-no-sandbox）。
...
| 存放     | `~/.apollo/skills/<name>/SKILL.md`             | `~/.apollo/plugins/<name>/`                     |
| 装载     | 冷启动扫 metadata，按需读全文                   | 显式 install + manifest 声明能力                |
| 隔离     | 不执行代码                                     | permission + manifest + 静态检查                |
| 版本     | `SKILL.md` frontmatter `apolloVersion: ^1.0.0` | `manifest.json` `engines.apollo: ^1.0.0`        |
...
| 名称                | 类型              | 主要能力                                                    | 落盘位置                          |
|---------------------|-------------------|-------------------------------------------------------------|-----------------------------------|
| `apollo-sandbox`    | 独立可执行二进制  | 执行 shell 命令 + syscall 隔离；**沙箱内执行插件 Node 子进程**  | `platforms/native-sandbox-*/bin/` |
| `apollo-search`     | napi-rs .node     | ripgrep 绑定 + tree-sitter 语法查询                          | `platforms/native-search-*/`      |
| `apollo-fs`         | napi-rs .node     | 大文件 diff / tokenize（tiktoken-rs 或 gpt-tokenizer）        | `platforms/native-fs-*/`          |

**为什么 `apollo-sandbox` 是独立二进制而非 napi addon**：
- 沙箱需要 fork/exec 新进程 + 挂载 syscall 过滤，Node 主进程内做会污染
- 独立二进制让 sandbox 有自己的进程生命周期，崩溃不影响 apollo 主进程
- 便于用户手动审查（`file apollo-sandbox` + 校验签名）
- **同一套框架同时服务 Bash 与 Plugin**：v3 修正后，插件也在 apollo-sandbox 子进程内跑（见 §5.3 `--run-plugin` 模式与 §6.4.3），避免"Bash 有 Rust 沙箱、插件没沙箱"的自相矛盾

### 5.3 apollo-sandbox 设计
...
CLI：apollo-sandbox exec  （或省略子命令，兼容默认）
...
职责：在沙箱内 execve 一个 Node 进程加载指定插件 index.js，
      通过预先打开的 fd 3 与父 apollo 进程做 JSON-RPC bridge

CLI：apollo-sandbox --run-plugin \
...
内部流程：
  ...
  4. execve 系统 node 二进制，argv = [node, <apollo-runtime>/plugin-host.mjs, <entry>, <dataDir>]
...
**profile 生成规则**（`crates/apollo-sandbox/src/profile.rs` 单一权威）：
...
│  ├─ sandbox.ts              # 封装 apollo-sandbox 子进程调用
│  ├─ search.ts               # 封装 apollo-search addon
│  ├─ fs.ts                   # 封装 apollo-fs addon
│  ├─ fallback/
...
resolveNative(kind):
  ...
  pkg = `@apollo-code/native-${kind}-${triple}`
  try:
...
| Sandbox binary **必须**是独立进程，不 dlopen 进 Node                                | crates/apollo-sandbox 是 bin，不是 lib          |
...
| Sandbox profile **禁止**放宽已在 PermissionSpec 声明的权限                          | crates/apollo-sandbox 单元测试                  |
| Rust crate **禁止**发布到 npm（只发平台产物包）                                     | pnpm workspace 配置                              |
| `apollo-search` 结果**必须**尊重 `.gitignore` / `.apolloignore`                     | search crate 单元测试                            |
| 插件宿主 **必须**通过 `apollo-sandbox --run-plugin` 启动，不允许 plugin-runtime 内直接 `child_process.spawn(node)` | ESLint + code review                             |
...
| 插件 RPC method **必须**在 `manifest.permissions.apollo` 白名单内，未声明直接拒绝    | plugin-runtime bridge 单元测试                    |

### 5.9 里程碑

- **L1（MVP）**：`apollo-sandbox exec` macOS + Linux；`apollo-search` 基础版；`native-bridge` 解析 + JS fallback；Windows 无沙箱提示 `--dangerously`
- **L2**：`apollo-fs`（diff + token 计数）；6 target 全 CI matrix；`apollo-sandbox --run-plugin` 模式落地（macOS + Linux），支持 §6.4.3 的 JSON-RPC bridge；plugin-runtime v1 集成
- **L3**：AST 查询（tree-sitter）；沙箱违规日志（`sandbox_violations`）；插件资源守护（`setrlimit` + 强制 kill）
- **L4**：Windows AppContainer 沙箱（Bash + 插件同时启用）；apollo-search 支持自定义 codec / 二进制识别
...
- `apollo.provider` — 插件不能直接调 provider
...
- Plugin：`plugin-runtime` 将 `apollo.prompt.contribute` 转发到 composer
...
```md
You are Apollo Code, an interactive terminal AI coding agent. You help the user
...
description: >                              # 必需，一句话（<200 字符）
  Guides Apollo to write conventional commits, use atomic changes, and
  document breaking changes clearly.
apolloVersion: ^1.0.0                       # 必需 semver
...
1. **冷启动扫描**（apollo 启动时）：`skills-runtime` 遍历 `~/.apollo/skills/*/SKILL.md` **只读 frontmatter**（yaml_front_matter 库前置停止），收集 `name` + `description` + `activation`。
2. **候选提示**（compose 时）：由 `skills-runtime` 向 PromptComposer 注册一个**索引 fragment**（priority=850），格式：
   ...
   - git-workflow: Guides Apollo to write conventional commits...
...
1. Apollo 启动时（cwd 已知），`storage` 从 cwd 向上遍历，每级找 `AGENT.md`（不跨用户 home 边界；最多向上 8 级即停）
...
5. Apollo 也读 `<cwd>/CLAUDE.md`（若存在），作为 fallback 兼容 claude-code 生态：
...
You are Apollo Code, an interactive terminal AI coding agent...
...
<!-- source: user:~/.apollo/PROMPT.md, priority: 400 -->
...
`<!-- source: ... -->` 注释放在文本里对模型无害（大部分 tokenizer 会作为普通 token），但对**用户 debug** 极有价值（`apollo debug prompt` 可 dump 这段）。

#### 6.5.6 `@include` 机制

**问题**：多个 md（AGENT.md / PROMPT.md / SKILL.md / memory）经常有共享片段（团队共享的编码规范、领域词表），复制粘贴容易漂移。

**决策**：apollo 内部**提示词加载管线**支持 `@include <path>` 语法。**只在提示词加载管线生效**，`Read` 工具读 md 时**不展开**（保留原文，避免污染搜索 / diff / edit 语义）。

**语法**（严格）：
- 一整行以 `@include ` 开头（行首无缩进），后面是路径，无引号；行尾允许 `# comment`
- 例：`@include ./shared/coding-style.md`
- 例：`@include ~/.apollo/shared/team.md   # 团队共享片段`
...
- 绝对路径 → 允许，但**规范化后**必须落在 workspace（`<cwd>/**`）或 `~/.apollo/**` 之一
...
| 规则 | 强制点 |
|---|---|
| `@include` **只在** apollo 内部提示词管线展开，`Read`/`Grep`/`Edit` 工具**不展开** | `PromptLoader` 与 `Read` 工具是不同代码路径；单元测试守 |
| 展开路径 **必须**落在 workspace 或 `~/.apollo` 双白名单内 | `PromptLoader.canonicalizeAndCheck()` 单元测试 |
...
### 6.6 SDK：`@apollo-code/plugin-sdk`
...
│  ├─ types.ts               # ApolloBridge / ToolSpec / HookEvent / PromptFragment ...
...
```ts
// 作者 TS 项目
import { definePlugin, defineTool } from '@apollo-code/plugin-sdk'

export default definePlugin({
  name: 'git-helper',
  version: '1.2.0',
  async activate(apollo) {
    apollo.tools.register(defineTool({
      name: 'git-status',
      // ... 全类型补全
    }))
  }
...
2. **`packages/plugin-runtime` 责任扩充**（v3 修正后）：
   ...
   - **通过 `native-bridge.runPlugin()` 起 `apollo-sandbox --run-plugin` 子进程**，不再是 `node:vm`
...
3. **`packages/native-bridge` 责任扩充**（v3 修正后）：
   - 新增 `runPlugin(opts) → PluginProcHandle` 原语：spawn `apollo-sandbox --run-plugin`，保留 fd 3 作为 bridge socket
...
4. **`packages/core` 责任扩充**：
   ...
   - 新增 `PromptLoader`（md 读入 + frontmatter 剥离 + `@include` 递归展开 + workspace/`~/.apollo` 双白名单，见 §6.5.6）
...
7. **新增运行时资源**：
   - `packages/plugin-runtime/runtime/plugin-host.mjs`（与 plugin-runtime 一起发布，`apollo-sandbox --run-plugin` execve Node 时加载它作为宿主脚本）
...
| 规则                                                          | 强制点                                    |
|---------------------------------------------------------------|-------------------------------------------|
| 插件文件系统访问 **必须**限定在 `pluginDir` 只读 + `dataDir` 读写   | `apollo-sandbox` profile（landlock/sbpl）  |
| 插件 **不允许** 任意 `net` / `exec` / `ptrace`                  | `apollo-sandbox` seccomp / sbpl           |
| 插件 **不允许** 运行时 `import('some-npm-pkg')`（子进程内无 node_modules 可读） | sandbox profile + plugin-host 起步无 loader hook |
| 插件所有 fs/exec/net 请求走 `apollo.*` bridge → permission     | plugin-runtime RPC dispatch               |
| 插件 RPC method 未在 `manifest.permissions.apollo` 白名单 → 拒绝 | plugin-runtime bridge 单元测试             |
...
| 插件子进程崩溃 → 隔离到该插件（不影响主进程和其它插件）           | `apollo-sandbox` 独立进程 + `error.raised` 事件 |
| 插件被禁用 / 卸载 → SIGTERM 子进程 + 释放所有 Disposable        | `plugin-runtime` 持有 disposable + procHandle 列表 |
| 插件不能持有 `SessionState` 引用（可能拿到旧版本）              | `apollo.session` 只返 immutable 快照（走 RPC 序列化天然做到） |
| 插件间不能互相通信 / 读取                                      | 每个插件独立子进程，独立 dataDir，独立 profile |
| 插件 stdout / stderr 环形 buffer + 尾部保留，转 telemetry       | apollo-sandbox 侧限流；plugin-runtime 记录 |

### 6.9 与其它扩展机制的关系

| 机制    | 语言   | 维护方   | 权限 | 主要用例                            |
|---------|--------|----------|------|-------------------------------------|
| Skill   | Prompt | 人工     | 无   | 教 apollo "怎么做 X"（知识/流程）    |
...
**依赖**：插件系统 L1 依赖 §5.9 的 L2（`apollo-sandbox --run-plugin` 落地）。因此 §6.10 的时序整体后移一个里程碑。

- **L1（依赖 §5.9-L2）**：manifest + 单文件加载 + `apollo-sandbox --run-plugin` 子进程 + JSON-RPC bridge + `tools.register` / `hooks.on` / `prompt.contribute` / `commands.register` / `apollo.log`；无 http / no exec（先只让插件贡献 prompt 和纯计算工具，收窄 sandbox profile 到最小面）
- **L2**：`apollo.fs`（读白名单） / `apollo.exec`（走 permission → 内层 `apollo-sandbox exec`，插件不直接开 shell）/ `apollo.http`（按 manifest.permissions.net 白名单）
- **L3**：升级检测（权限变化再确认）+ 热插拔 enable/disable + 资源守护（`setrlimit` + bridge 调用次数/延迟限制）
- **L4**：`apollo plugin dev` 开发模式（`--dev` 分支 profile 放宽 + hot reload watcher）+ registry search（延后 v2）+ Windows AppContainer 插件宿主
...
`apollo.hooks.on(event, handler, opts?)` 签名扩展：

```ts
apollo.hooks.on('preToolUse', handler, { priority: 10 })
```
...
**v3 修正后**：插件跑在 `apollo-sandbox --run-plugin` 独立子进程内，主进程 100% 不受影响。资源守护变成"什么时候杀这个子进程"的问题，而非"能不能杀"。
...
| L4 | **进程级 `setrlimit`**（apollo-sandbox 在 execve 前设置） | CPU 时间 / RSS / 文件句柄 超限 → 内核 kill，父感知 exit code |
| L5 | **无响应心跳**：apollo 每 30s 发 `ping` RPC，60s 无响应 → 判定挂起 → `SIGKILL` | 记 `error.raised` |
| L6 | 用户主动 `apollo plugin ban <name>` | 加入 `~/.apollo/plugins.banned.toml`，永久拉黑 |
| L7 | 3 次连续 `activate` 失败 → 自动 disable | 需要 `apollo plugin enable <name>` 显式恢复 |
...
- ✅ 崩溃隔离（Node segfault / OOM 不影响主 apollo）

**仍不做的**：
- 网络流量整形（应用层记 telemetry，不做速率限制）
- 磁盘配额（依赖平台文件系统，v2+）

**接受的风险**：装恶意插件本就是信任行为；apollo 通过 Rust sandbox + 白名单 bridge + manifest + 资源上限做到"意外失控完全隔离、恶意攻击者提高攻击成本"。这与 §6.4.3 的心智一致。

#### 6.11.3 插件诊断

新增 CLI 命令 `apollo plugin doctor <name>`，输出：

```
Plugin: apollo-plugin-git-helper@1.2.0
Status:              ✅ enabled
Load time:           123 ms
Manifest:            ✅ valid, engines.apollo=^1.0.0 (current 1.0.3, ✓)
...
Recent errors:       (last 5, if any)
  2026-07-31 12:34:56  bridge timeout: apollo.exec > 5s
...
补充 `apollo.session.on` 可订阅的 session 事件（供插件跟踪 session 生命）：
...
同时补 `apollo.plugin.on`（插件自己的生命周期）：
- `deactivate.before` — 用户禁用/卸载前
- `permissions.changed` — 用户在 UI 里改了权限（罕见但需要）

#### 6.11.5 插件升级迁移

**问题**：老 apollo 装了 plugin@1.0，用户升到 apollo@2.0，plugin `engines.apollo: ^1.0.0` 匹配失败 → 加载被拒 → 用户懵。

**策略**：
1. 升级 apollo 时，`apollo` 启动阶段扫描已装插件，对每个 mismatch 的插件：
   - 查询 npm registry（用 `plugin-registry` v2 或 npm search）看是否有兼容新 apollo 的更高版本
   - 若有 → 提示 `apollo plugin upgrade <name>`
   - 若无 → 保留在 disabled 状态，session 内红条提示"以下插件需作者更新才能兼容新版本 apollo"
2. `apollo plugin upgrade <name>` = `pnpm add <name>@latest` 到 plugin 目录 + 重跑权限确认
3. `apollo plugin upgrade --all` 批量

不做**自动降级 apollo** 或**兼容 shim**，避免生态碎片化。
...
| 层 | 路径（默认，可配） | 语义 |
|---|---|---|
| **全局** | `~/.apollo/memory/*.md` | 跨项目通用的长期偏好、常识 |
| **项目** | `<cwd>/.apollo/memory/*.md` | 当前项目专属；随仓库走（用户自行决定是否 `.gitignore`） |

**配置覆写**（`config.toml`）：

```toml
[memory]
paths.global = "~/.apollo/memory"              # 可改成团队共享盘
paths.project = ".apollo/memory"               # 相对 cwd
max_body_lines = 200                            # 单文件正文行数上限（不含 frontmatter）
max_files_per_scope = 500                       # 每层文件数上限（防失控）
recall_topk = 8                                 # apollo.memory.recall 默认返回条数
...
Storage: ~/.apollo/memory (global) and <cwd>/.apollo/memory (project).
...
### How to store
- Use `apollo.memory.write({ scope, title, tags, body, pinned? })`.
...
- Prefer updating an existing memory (`apollo.memory.update(id, patch)`) over creating a near-duplicate; use `apollo.memory.recall(query)` first to check.

### How to recall
- `apollo.memory.recall(query, { scope?, tags?, topk? })` at the start of a task or when a topic surfaces.
- Read full memory with `apollo.memory.read(id)` only when snippet is insufficient.
...
**用户 / 插件覆盖**：
- 用户覆盖：`~/.apollo/memory-guide.md`（存在则**替换**内置版；priority 保持 950）
- 项目覆盖：`<cwd>/.apollo/memory-guide.md`（叠加在用户版之上，同 priority slot 冲突时项目胜出）
- **插件贡献**：`apollo.memory.contributePrompt(fragment)` — priority 固定 `plugin:<name>:memory` = 60（在 plugin 常规 50 之上，但仍低于 project/user）；插件可**追加**建议但**不能**覆盖内置

#### 6.12.4 200 行限制（软 → 硬 → 可覆盖）

**分层策略**：
1. **软提示（模型侧）**：memory system prompt 里教模型上限（§6.12.3）
2. **硬校验（writer 工具侧）**：`apollo.memory.write` 提交时统计 body 行数，超限 → 返回 `MemoryLimitExceededError` 给模型，含明确 remediation：`{ current: 234, limit: 200, suggestion: 'split by heading or reduce examples' }`；模型收到后自行分段重试
...
#### 6.12.5 Bridge API（`apollo.memory`）

```ts
interface MemoryBridge {
  // 召回：按 query 做 BM25 / 关键字匹配（走 native-bridge 的 apollo-search）
...
#### 6.12.7 CLI（`apollo memory <sub>`）

```
apollo memory list [--scope global|project|both] [--tag T] [--pinned]
apollo memory show <id>                     # 输出 md（含 frontmatter）
apollo memory add [--scope ...] [--pinned]  # 交互 / --title / --tags / --body-stdin
apollo memory edit <id>                     # 打开 $EDITOR，保存时 re-validate frontmatter + limit
apollo memory rm <id> [--yes]
apollo memory pin <id> / unpin <id>
apollo memory search <query> [--scope ...] [--tag ...]
apollo memory export [--scope ...] > memory.tar
apollo memory import memory.tar             # 冲突：--strategy skip|overwrite|rename
apollo memory doctor                        # 校验所有 memory frontmatter / 行数 / 冲突
apollo memory reindex                        # 重建搜索索引
```
...
| priority | source | 说明 |
|---|---|---|
| 1000 | `builtin` | Apollo 内置基础 prompt（§6.5.1） |
...
- **`packages/memory-runtime`**：memory 存储 / 索引 / bridge server（供 plugin-runtime bridge 转发调用） / CLI 后端
  - 依赖：`core[type-only]` + `permission` + `hooks` + `native-bridge`（`apollo-search` 做召回索引） + `shared`
...
`§1.2 依赖表` 增补：
- `memory-runtime → core[type-only] / permission / hooks / native-bridge / shared`
- `plugin-runtime` 通过 apollo bridge 转发 `memory.*` RPC 到 `memory-runtime`（不新增 import 依赖，走运行时端口）
...
| frontmatter schema 校验失败 → **拒绝加载**，emit warning，用户可 `apollo memory doctor` 修 | 单元测试 |
...
| `memory.doctor_fixed` | `apollo memory doctor` 修复了 N 条 |
...
- **L3**：`recall` 走 apollo-search 索引 + hooks 全谱 + 插件 `apollo.memory.*` bridge 暴露 + `apollo memory doctor`
...
`@` 前缀在 apollo 中承担**两种能力**（引用文件 / 覆盖模型），语义在输入时**由用户主动选择**，避免启发式歧义：
...
2. 选中后 popup 切换为对应的**二级补全器**：
   - **file 模式**：popup 变为 fuzzy path picker，候选来源为 cwd（尊重 `.gitignore` + `.apolloignore`），选中后在输入行插入 chip `[@file:<relative-path>]`
...
**心智**：auth 是安全 + 用户体验的双热点，必须为**后期数据分析**（不是自动上报）留下完整轨迹。所有事件写入本地 `~/.apollo/telemetry/*.jsonl`，遵循 §4.13：默认不出网，仅 OTel opt-in 时上报。**每一条事件都必须脱敏**（不含 key/token/passphrase 明文）。
...
- `Write` / `Edit` / `MultiEdit` 执行前，如果目标文件存在 → 备份到 `~/.apollo/backups/<sha>/<original-path>`
- `apollo restore <session-id>` 可回滚该 session 内的所有变更
- Backups 有 GC 策略：默认保留 7 天 + 500MB 上限，超出按 LRU 清理

### 8.7 Telemetry：默认本地

**已在 §4.13 强制**：默认写 `~/.apollo/telemetry/*.jsonl`，不出网。
...
| Rust addon    | **napi-rs**（apollo-search / apollo-fs）         |
| Rust bin      | **cargo** + `cross`（apollo-sandbox 交叉编译）    |
...
3. 合入 Version PR → tag 触发 CI `release` job：
   ...
   - `apps/cli` 单独发 `apollo-code` 包（bin）
...
- ✅ native-bridge + apollo-sandbox（macOS + Linux）+ apollo-search 基础
...
**Definition of Done**：能用 `apollo` 对着 Anthropic 完成一个真实的编码任务（改文件 + 跑测试 + 提 PR），全程权限弹窗可控。
...
- ✅ JSBridge 完整 API 表面（apollo.tools/hooks/commands/prompt/session/fs/exec/http/ui/storage/config/log）
...
Email: security@apollo-code.dev (or maintainer email if not set up)
...
├─ ISSUE_TEMPLATE/
│  ├─ bug_report.yml                # 结构化：期望 vs 实际 / apollo doctor 输出 / 复现步骤
│  ├─ feature_request.yml
...
- **主域名**：`apollo-code.dev`（`.dev` 域强制 HTTPS + 品牌感）
- **备选**：`apollocode.io` / `apollo.dev`（需查商标 + 注册占用）
- **部署**：
  - MVP：GitHub Pages + Cloudflare（免费）
  - L2+：Vercel（PR preview + edge functions）
- **备用文档站**：`main.apollo-code.dev` / `next.apollo-code.dev` 分别对应 main / next 分支
...
```
┌───────────────────────────────────────────────────────────────────────┐
│ [APOLLO CODE logo]  Docs  Plugins  Blog  GitHub ⭐ 12.3k     [Install]│  ← Sticky nav
├───────────────────────────────────────────────────────────────────────┤
│                                                                       │
...
│                                                                       │
│         ┌─────────────────────────────────────────────┐              │
│         │ $ npm install -g apollo-code                │  ← One-liner  │
│         │ $ apollo                                     │     copyable │
│         └─────────────────────────────────────────────┘              │
│                                                                       │
...
│   npm i -g apollo    apollo login anthropic   apollo "fix this bug"   │
│                                                                       │
├───────────────────────────────────────────────────────────────────────┤
│                        See it in action                                │
│                                                                       │
│   [ Short 30-second GIF: apollo does a real task ]                    │
│   Caption: "Apollo refactoring 3 files, running tests, opening PR"    │
│                                                                       │
├───────────────────────────────────────────────────────────────────────┤
│              Built for developers who want control                    │
│                                                                       │
│   Concepts diagram: [ user → apollo → router → provider ]             │
│                       [           ↑ sandbox     ]                     │
│                                                                       │
...
│                                                                       │
│  © 2026 Apollo Code contributors. Licensed under Apache-2.0.          │
└───────────────────────────────────────────────────────────────────────┘
```
...
| 元素 | MVP 决策 |
|---|---|
| Logo | 简单字标 `apollo` 或几何图形（宇航头盔 / 神庙 / 太阳） |
| 主色 | 深蓝（#0E1729）+ 太阳金（#F4B942）—— 呼应 "Apollo" |
...
- **不加**：Google Analytics（隐私 + 与 apollo 隐私价值观矛盾）
...
用户 `npm i -g apollo-code` 后第一次跑 `apollo` 时的完整体验。
...
- 验证通过才写 keychain
- 建议加 `--api-key-stdin`：`echo $KEY | apollo login anthropic --api-key-stdin` 让脚本用户跳过 TUI
...
任一失败 → 让用户选加密文件 or env 模式，**明确告知**"encrypted file at ~/.apollo/credentials.enc, unlock with passphrase"。

**隐私首屏声明**（step 1 前一个屏）：

```
Before we start:
  • Apollo saves session logs LOCALLY to ~/.apollo/sessions/
  • Apollo does NOT send analytics anywhere by default
  • Your prompts and code are only sent to the provider you choose
  • You can review or disable anytime: apollo telemetry status
...
用户在一个新项目里第一次跑 `apollo`：

```
$ cd my-project && apollo
...
**AGENT.md 生成（选项 2，L2）**：apollo 主动扫代码（README / package.json / 主要文件），生成初版 AGENT.md 供用户 review + save 到 `<cwd>/AGENT.md`。类似 claude-code 的 `/init`。等 skills-runtime 完备后再上线，避免 L1 阶段扫描逻辑没沉淀就先做。
...
- 用户中途 Ctrl+C → 已保存的 credential 保留，未完成的步骤下次 `apollo` 时从中断处继续
- `apollo --reconfigure` 强制重跑 onboarding
...
```
$ apollo chat "review this diff" < diff.txt

apollo: WARNING: no provider configured. Non-interactive mode.
        Set APOLLO_PROVIDER=anthropic and APOLLO_ANTHROPIC_API_KEY,
        or run `apollo login` first in an interactive shell.
        Exit code: 2
```
...
- **L4**：`apollo --reconfigure` + onboarding 分支：Ollama 无 key 快速路径 / API key OAuth flow

## Future / v2

- Auto-update 机制（`apollo update`）
...
| 2026-07-31 | §5 v1 | Rust 侧车：三个产物（apollo-sandbox 独立 bin / apollo-search napi / apollo-fs napi）；平台沙箱实现（macOS sandbox-exec / Linux landlock+seccomp / Windows AppContainer 延后）；native-bridge 单点接入 + JS fallback；attachment handle 生命周期；沙箱 profile 从 PermissionSpec 生成；边界与安全清单 |
...
| 2026-07-31 | §8 v1 | 存储：纯文件（`~/.apollo/` + `<cwd>/.apollo/`）；session JSONL append-only（stream.delta 不落盘）+ replay；config.toml 分层（内置<全局<项目<env<flag）；credentials 三层 fallback（keychain / 加密文件 / env）；backups + GC；telemetry 默认本地；边界与安全清单 |
...
| 2026-07-31 | §6 v2  | 插件补漏：6.11.1 hook 多插件执行顺序（numeric priority + 串行 pipeline + veto 短路）；6.11.2 资源守护 5 层（activate 10s / bridge call 5s / 500 calls/turn / ban / auto-disable）；6.11.3 `apollo plugin doctor` 诊断命令；6.11.4 生命周期事件（session.start/end + permissions.changed）；6.11.5 升级迁移（manifest.migrations + apolloVersion 校验） |
...
| 2026-07-31 | §13 v1 | 文档站 IA：域名 `apollo-code.dev`；VitePress 8 部署；完整站点信息架构（getting-started/concepts/guides/reference/cookbook/troubleshooting/plugins/skills/blog/changelog/roadmap/community）；官网首页 wireframe（hero + 6 特性 + 3 步上手 + demo GIF + 信任声明 + 社区）；MVP 品牌与视觉基线；CLI/API/config 自动生成；无追踪隐私默认 |
...
| 2026-07-31 | review r1 | Self-review 修正一轮：§10 L1 "5 工具" → "7 工具"；§4 v1 changelog 澄清 MultiEdit L2 / Task 归 L3；§1.2 skills-runtime 补 `core[type-only]` 依赖 + auth→native-bridge 依赖说明为可选运行时；§11.2 顶层命令补 `completion`、`update` 明确归 v2、总数订正为 17；§11.3.9b 新增 `apollo hook` 子章节；§11.3.10 doctor 输出按 L1/L2/L3/L4 分层展示 + `--json`/`--strict` flags；§11.3.2 `apollo login` 明确 verify-first-save-second + `--skip-verify` 需 `--dangerous`；§11.7 里程碑同步；§14.2 补 Ollama 分支流程；§14.4 mockup 标 `(L2)` + "信任 cwd" 范围限定为 read/search，write/exec 仍走 permission；§6.5.4 AGENT.md priority 递减公式（`600 - 10*level`，下限 500，CLAUDE.md fallback 复用同槽位）；§10 加时间估算前提说明；§12.5 CONTRIBUTING.md 去重改为引用仓库文件 + 独立 12.5b RFC 触发清单；SECURITY.md `Supported versions` 表订正为 0.x pre-release 现实语义；CONTRIBUTING.md 分层 branching model（L1-L2 trunk-based，L3+ 引入 `next`） |
| 2026-07-31 | review r2 | **架构一致性修正**（用户反馈：Rust 沙箱应统一到插件，不该用 `node:vm`）：§6.4.3 重写：`node:vm` 方案作废，改为 `apollo-sandbox --run-plugin` 独立子进程 + JSON-RPC 2.0 over NDJSON (fd 3) bridge；每插件一 Node 子进程，profile 从 `manifest.permissions` 生成；附件走 handle-token 模式（native pointer 不出主进程）。§5.2 apollo-sandbox 能力表补"沙箱内执行插件 Node 子进程"。§5.3 拆为 `exec` 与 `--run-plugin` 两模式（3 段：5.3.1/5.3.2/5.3.3），共用 profile 生成与平台机制；插件模式额外禁 `ptrace`/`mmap w+x`。§5.8 边界清单新增 4 条（sandbox 唯一入口 / profile 由 native-bridge 生成 / 崩溃隔离 / RPC 白名单）。§5.9 里程碑：`--run-plugin` 归 L2；插件资源守护归 L3；Windows 插件宿主归 L4。§6.7 差量：plugin-runtime 新增 `native-bridge` 依赖 + 新增 `packages/plugin-runtime/runtime/plugin-host.mjs` 宿主脚本；`native-bridge` 新增 `runPlugin(opts) → PluginProcHandle` 原语。§6.8 安全清单重写（sandbox 语义替代 vm 语义 + 新增子进程崩溃隔离/stdout 限流规则）。§6.10 里程碑标注依赖 §5.9-L2。§6.11.2 资源守护由 5 层扩展到 7 层（新增 `setrlimit` + 心跳），且现在真正做到 CPU/RSS/fork 上限。§6.11.3 doctor 输出：VM state → Sandbox subprocess pid/rss/cpu + profile。§1.2 依赖表：`plugin-runtime` 新增 `native-bridge` 依赖。心智：AST 静态检查从"安全底线"降级为"作者友好检查"，真正的隔离靠 Rust sandbox + JSON-RPC 白名单 + manifest 三层。 |
| 2026-07-31 | review r3 | **一等公民规则 + auth 事件谱**：新增 §8.4.1 auth telemetry 事件谱（17 类事件：`auth.login.started` / `verify_requested` / `verify_result` / `stored` / `failed` / `cancelled` / `logout.completed` / `credential.resolved` / `credential.miss` / `keychain.error` / `encfile.unlock_prompted` / `encfile.unlock_result` / `migration.plaintext_found` / `migration.plaintext_moved` / `dangerously.skip_verify` / `mcp.keyref_created` / `mcp.plaintext_kept`），字段全部经 `shared.sanitize()` 脱敏；本地 sink 为默认，OTel opt-in；§8.7 补 2 条边界规则（分支必发事件 + payload 必脱敏）。AGENT.md §1.1 新增"一等公民架构（不可动摇）"两条：Rust 沙箱是安全基座、TS 是开发效率基座；§4.6 补 verify-first-store-second + MCP keyref + auth 事件上报硬约束；§4.7 重写为"Rust 沙箱 = 一等公民"，含适用范围表 + 平台机制 + 降级 + 永远禁止清单；§4.10.1 抽掉 `node:vm` 语义，改为 `apollo-sandbox --run-plugin` 子进程 + JSON-RPC bridge 描述；§4.1 依赖图 plugin-runtime 加 native-bridge。CLAUDE.md §C0 新增"一等公民架构"镜像；§C3 plan mode 触发条件新增沙箱边界 / auth 事件谱；§C4 禁止事项去除过时 `vm.Context` 条，改为禁止 `node:vm` / `worker_threads` / 手动 profile / 绕过 verify-first / auth 事件缺失 / payload 未脱敏，共 5 条新增。 |
...
| 2026-07-31 | review r4 | **Memory 系统 + `@include` 机制**（用户反馈：需要跨会话记忆 + md 文档递归引用）：spec 新增 §6.12 Memory 系统（12 小节：配置 / md 文件格式 / memory-guide 内置提示词 / 200 行限制的 4 层降级链 / MemoryBridge API / hook 谱 / `apollo memory` CLI 子树 / 召回策略 / 新增 `packages/memory-runtime` 包 / pinned auto-inject / 与 Skill/AGENT.md 的边界 / 里程碑）；spec 新增 §6.5.6 `@include` 机制（语法 / 路径解析 / workspace+`~/.apollo` 双白名单 + canonicalize+symlink 逃逸检测 / 递归深度 8（可配 32）/ 单 compose 展开次数 64 上限 / cycle 检测 / frontmatter 剥离 / debug marker / 错误占位不中断 compose / telemetry 事件 `prompt.include.expanded` + `prompt.include.failed` / 实现单点 `packages/core/src/prompt-loader.ts`）；§1.1 package tree 补 `memory-runtime/` + `plugin-runtime` 注释同步为 sandbox 子进程语义；§1.2 依赖表补 `memory-runtime` 行 + ASCII 图 memory-runtime 注入行；§6.7 差量补 `PromptLoader` 责任 + 三 contributor 共用 loader + 内置 `memory-guide.md` 资源 + `packages/memory-runtime` 新包；§6.9 对比表由 4 行扩到 6 行（加 Memory + AGENT.md），列头改"语言/维护方"，决策语说明 Skill vs Memory vs AGENT.md 三者取舍；§11.2 顶层命令 17 → 18（加 `apollo memory`），§11.3 交叉索引到 §6.12.7；§8.2 存储布局树补 `~/.apollo/memory/*.md` + `<cwd>/.apollo/memory/*.md` + `index.jsonl` 摘要索引；§8.7 边界清单补 4 条（frontmatter 校验 / body 200 行 / 路径 canonicalize+escape / `apollo.memory.*` 过 manifest.permissions）。AGENT.md §4.1 依赖图补 `memory-runtime`；新增 §4.14 Memory 硬约束（存储位置固定 / frontmatter 必填字段 / 200 行 4 层降级 / 模型主导 / 唯一召回路径 / 权限门 / 4 条禁止）；新增 §4.15 `@include` 硬约束（仅 apollo prompt 管线 / 单点实现 / 只 md / 双白名单 / 递归安全 / frontmatter 剥离 / 错误不中断 / debug 可观测 / 3 条禁止）。CLAUDE.md 顶部 §4.14/§4.15 索引 + §C4 禁止事项新增 12 条（memory-runtime 单点 / frontmatter / 200 行 4 层 / 无 raw key / 无另起提示词 / 权限门 / prompt-loader 单点 / 只 md / 仅 prompt 管线 / 双白名单 / 修改 max_body_lines 需同步 guide）；§C7 提示触及 memory / `@include` 需先读 §6.12 / §6.5.6。 |
