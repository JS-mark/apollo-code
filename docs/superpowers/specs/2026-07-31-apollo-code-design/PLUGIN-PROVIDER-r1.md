...
# Apollo Code · 插件 Provider 扩展白皮书 (r1)

> **状态**：Approved（2026-08-01）
> **文档类型**：架构决策记录 (ADR) + 扩展规约
> **范围**：`packages/provider-kit` / `packages/router` / `packages/plugin-runtime` / `packages/plugin-sdk` / `ApolloBridge.provider`
...
1. **受控的 provider 扩展**：允许第三方插件 out-of-tree 注册 `ProviderClient` 实现，填补 apollo 扩展性矩阵里唯一的硬缺口（见 [§6.9](./06b-prompt-composer.md) 对比表）。
...
### P2.1 vs §6.4.1 "明确不暴露 apollo.provider"

**原约束**：`apollo.provider` 不存在（插件不能直接调 provider，防绕过 router / cost tracking）。

**张力**：本白皮书**新增** `ApolloBridge.provider` 命名空间，让插件**注册** provider。
...
- 命名空间从"禁止"变"受控开放"：`apollo.provider.register(spec)` 是唯一 API，不暴露 `apollo.provider.<name>.stream()` 直调入口。
...
- **plugin**：`plugin-runtime` 加载 `kind: 'provider'` 插件、`activate()` 内调 `apollo.provider.register(spec)` 时注册。
...
```
main → child:  {"jsonrpc":"2.0","id":7,"method":"apollo.provider.stream.open",
                "params":{"providerName":"plugin-vllm","req":{...}}}
child → main:  {"jsonrpc":"2.0","method":"apollo.provider.stream.chunk",
                "streamId":"s_7","params":{"chunk":{"kind":"text.delta","text":"H"}}}
child → main:  {"jsonrpc":"2.0","method":"apollo.provider.stream.chunk",
                "streamId":"s_7","params":{"chunk":{"kind":"text.delta","text":"i"}}}
...
child → main:  {"jsonrpc":"2.0","method":"apollo.provider.stream.end",
                "streamId":"s_7"}                      // 正常结束
                // 或 {"...","method":"apollo.provider.stream.error","params":{"err":{...}}}
main → child:  {"jsonrpc":"2.0","method":"apollo.provider.stream.abort",
...
**流程**：
1. 用户 `apollo login plugin-vllm --api-key-stdin` → key 进 auth（keychain / enc / env），scope = `plugin-vllm`。
...
**强制**：ESLint + code review 守"main 才能读 raw key"；子进程内 ProviderClient 代码拿不到 `auth` bridge 的 getCredential（`apollo.auth` 不对 provider plugin 暴露 raw 读取，见 §P7.1）。
...
**流程**：
1. 用户 `apollo login plugin-vllm --signing` → main 把 secret 存 auth。
...
`ApolloBridge` 新增 `auth`（**只读、scope 限定、不暴露 raw key**）：
...
## §P7 ApolloBridge.provider 命名空间（新增 API）

```ts
// §6.4.1 ApolloBridge 表新增
apollo.provider: {
  /**
...
```jsonc
{
  ...
  "name": "apollo-plugin-provider-vllm",
  "version": "1.0.0",
  ...
  "permissions": {
    ...
    "apollo": ["provider.register", "auth.getAuthHeaders", "log"]
...
- `apollo model use plugin-vllm/llama-3.1-70b`（设默认）→ **v1 拒绝** + 提示"插件 provider 不能设为默认（v1），可用 `@alias` 单次调用或 config fallback"。
- v2 评估放开（届时加高亮确认）。
...
### P11.2 §6.4.1 ApolloBridge 新增 provider 命名空间

表末追加 `provider` 行（API 见 §P7），并把"明确不暴露"段 `apollo.provider` 那条改为：
> - `apollo.provider` 的**直调入口**（如 `stream`/`complete`/`getCredential`）不暴露；**仅** `apollo.provider.register(spec)` 受控开放（见 [PLUGIN-PROVIDER-r1](./PLUGIN-PROVIDER-r1.md)）
...
- CLAUDE.md §C4 禁令：原"禁止暴露 `apollo.provider`"改写为"禁止暴露 provider **直调**入口；`apollo.provider.register` 受控开放（见 PLUGIN-PROVIDER-r1）"。
...
| 规则 | 强制点 |
|---|---|
| 插件 provider **必须**经 `apollo.provider.register` 进 Registry，禁止任何直调 stream/complete 的旁路 | plugin-runtime RPC dispatch 白名单 |
...
| 插件 provider **不能**设为 default（v1） | `apollo model use` 校验 |
...
| 日期 | 版本 | 内容 |
|---|---|---|
| 2026-08-01 | r1 | 初稿：用户需求"引入受控 ProviderRegistry 端点，让插件注册 ProviderClient"。三大决策（用户拍板）：D1 sandbox 子进程 + 专用 stream 通道 / D2 main 注入凭据（header-template 默认 + signing 降级）/ D3 显式配置才进 Router 不能 default。13 节：目标/张力消解/Registry 端口/stream-over-RPC/凭据分层/ApolloBridge.provider/manifest kind/边界B1-B8/风险S1-S5/Router集成/§1§3§6差量/里程碑(v2)/清单。逆转 §6.4.1 "不暴露 apollo.provider" 与 §3.10 "Runner 禁 import provider-*" 两条原绝对约束为"受控开放"。归 v2 候选里程碑。 |
