...
│  ├─ apollo-sandbox/             # 独立二进制：syscall 隔离执行器 + 插件宿主
│  ├─ apollo-search/              # 独立二进制 worker：ripgrep + tree-sitter（常驻 IPC）
│  ├─ apollo-fs/                  # 独立二进制 worker：大文件 diff / tokenize（常驻 IPC）
│  └─ xtask/                      # Rust 侧构建/发布脚本
│
├─ platforms/                     # 各平台 prebuild 二进制的独立发布包（见 §1.6；产物均为独立二进制；L1 12 包 = 4 target × 3 crate，L2 扩 24 包 = 8 target × 3 crate）
│  ├─ native-sandbox-{darwin-arm64,darwin-x64,             # apollo-sandbox 二进制
│  │                   linux-x64-gnu,linux-arm64-gnu,      # ← L1 这 4 target
│  │                   linux-x64-musl,linux-arm64-musl,
│  │                   win32-x64-msvc,win32-arm64-msvc}/    # ← L2 补这 4 target
│  ├─ native-search-{同上 8 triple}/                        # apollo-search worker 二进制
│  ├─ native-fs-{同上 8 triple}/                            # apollo-fs worker 二进制
...
```
platforms/
  native-sandbox-<triple>/       → 打包 apollo-sandbox 可执行二进制
  native-search-<triple>/        → 打包 apollo-search worker 二进制（r9: 原 napi .node 作废）
  native-fs-<triple>/            → 打包 apollo-fs worker 二进制（r9: 原 napi .node 作废）
```
...
**共 3 crate × 8 target = 24 个平台包**（L2 全平台口径；产物均为独立二进制）。**L1 先发 12 包（4 target × 3 crate：darwin-arm64/darwin-x64/linux-x64-gnu/linux-arm64-gnu）**，L2 扩 24 包（补 Windows + musl）。另含跨平台共享的 `@apollo-code/native-fs-common`（BPE 表，非平台包，fs worker 启动时读）与可选的 universal2 合并包（sandbox/search/fs 各一，Homebrew 用；r9 后 search/fs 也可 lipo 合并，napi-rs universal 限制已解除）。
...
```json
{
  "optionalDependencies": {
    ...
    "@apollo-code/native-sandbox-linux-arm64-gnu": "workspace:*",
    "...（L1 共 12 项 = 上述 4 target × 3 crate）": "workspace:*",
    "@apollo-code/native-sandbox-linux-x64-musl": "workspace:*",
    "@apollo-code/native-sandbox-linux-arm64-musl": "workspace:*",
    "@apollo-code/native-sandbox-win32-x64-msvc": "workspace:*",
    "@apollo-code/native-sandbox-win32-arm64-msvc": "workspace:*",
    "...（L2 补齐至 24 项 = 8 target × 3 crate）": "workspace:*"
  }
}
```

**运行时解析（r9 改造）**：`native-bridge` 通过 `process.platform + process.arch + libc` 拼出目标包名（含 gnu/musl 后缀），`require.resolve` 拿到平台包目录 → 读 `package.json.bin` 字段拿到二进制路径 → `WorkerPool.spawn`（search/fs）或子进程调用（sandbox）。找不到就回落：search/fs → JS fallback（`apollo-search` fallback = fast-glob + JS 正则）；sandbox → tier=none + 提示 `--dangerous-no-sandbox`。详见 §5.8。
