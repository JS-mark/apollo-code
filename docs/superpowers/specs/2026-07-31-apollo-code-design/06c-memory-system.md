...
| **调优**（r10） | `~/.apollo/memory/*.md`（`scope: 'tuning'`） | 自我进化系统（[§15](./15-self-evolution.md) Layer A）写入的模型可读模式/偏好/教训；与 global/project 同目录，靠 frontmatter `scope` 区分 |
...
> 解决 REVIEW-r7 NEW-P1-C：§6.12.3 memory-guide（注入**模型** system prompt）教模型用 `apollo.memory.write/recall`，但 `apollo.memory.*` 是**插件 bridge** API（模型拿不到 `apollo` 对象）。模型只能调 ToolRegistry 工具。需定义模型面工具，否则 memory 对模型不可用（pinned 自动注入有效，但主动 recall/write 路径断了）。
...
|---|---|---|---|---|---|
| `Memory.recall` | ✅ | 无 | `{ query: string, scope?: 'global'\|'project'\|'both', tags?: string[], topk?: number }` | BM25/关键字召回，返回 `{ id, scope, title, tags, snippet }[]`（snippet 受 `recall_snippet_lines` 限） | `apollo.memory.recall` |
| `Memory.read` | ✅ | 无 | `{ id: string }` | 读全文（含 frontmatter） | `apollo.memory.read` |
| `Memory.write` | ❌ | 无 | `{ scope, title, body, tags?, pinned? }` | 写入；过 `memory.preWrite` 脱敏 hook + 200 行硬校验；超限返回 `MemoryLimitExceededError` remediation | `apollo.memory.write` |
| `Memory.update` | ❌ | 无 | `{ id, title?, body?, tags?, pinned? }` | 部分更新 | `apollo.memory.update` |
| `Memory.delete` | ❌ | 无 | `{ id }` | 删除 | `apollo.memory.delete` |
| `Memory.list` | ✅ | 无 | `{ scope?, tag?, pinned? }` | 列表（摘要） | `apollo.memory.list` |
...
**memory-guide 文案同步**：§6.12.3 内置 memory-guide 提示词里所有 `apollo.memory.*` 调用示例**改为**对应工具名（`Memory.recall` / `Memory.write` 等），与模型实际可调的 API 对齐。
