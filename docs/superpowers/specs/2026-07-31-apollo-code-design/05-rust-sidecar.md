...
| **零系统工具依赖**  | **用户无需装任何系统工具**；沙箱所需一切随 apollo-code 自带（Linux 的 bundled bwrap 编译时嵌入 + SHA256 校验，非 shell out） |
...
| 名称                | 类型              | 主要能力                                                    | 落盘位置                          |
|---------------------|-------------------|-------------------------------------------------------------|-----------------------------------|
| `apollo-sandbox`    | 独立可执行二进制  | 执行 shell 命令 + syscall/SID 隔离；**沙箱内执行插件 Node 子进程**；`--probe` 输出 tier | `platforms/native-sandbox-*/bin/` |
| `apollo-search`     | 独立可执行二进制  | ripgrep 绑定 + tree-sitter 语法查询；**常驻 worker 模式 + IPC** | `platforms/native-search-*/bin/`  |
| `apollo-fs`         | 独立可执行二进制  | 大文件 diff / tokenize（tiktoken-rs）；**常驻 worker 模式 + IPC** | `platforms/native-fs-*/bin/`      |

> **r9 架构变更**：`apollo-search` / `apollo-fs` 从 napi-rs `.node` addon 改为**独立二进制常驻 worker**（与 `apollo-sandbox` 形态统一）。理由：(1) 消除 napi ABI 版本依赖，提升安装可靠性（历史性痛点）；(2) 三产物架构统一，降低维护复杂度；(3) universal2 限制解除（napi-rs 不支持 universal `.node` 的硬约束失效，darwin 可 lipo 合并）。代价：search/diff/tokenize 调用走 IPC 往返 ~1-5ms/次（可接受，非热路径）。详见 §5.6.2 IPC 协议。

**为什么三个产物都是独立二进制**：
- 沙箱需要 fork/exec 新进程 + 挂载 syscall 过滤 / SID / AppContainer，Node 主进程内做会污染
- 独立二进制让每个产物有自己的进程生命周期，崩溃不影响 apollo 主进程
- 便于用户手动审查（`file apollo-sandbox` + 校验签名）
- **同一套框架同时服务 Bash / Plugin / Search / FS**：v3+ 修正后，插件也在 apollo-sandbox 子进程内跑（见 §5.3 `--run-plugin` 模式与 §6.4.3），search/fs 各自常驻 worker，避免"最需要沙箱的地方反而没沙箱"或"napi addon 一崩主进程跟着崩"的自相矛盾
- **同一套二进制既能被 native-bridge 派生子进程，也可以被用户直接 `apollo-sandbox --probe` 调试**，Runtime 与 CLI 语义合并
...
职责：在沙箱内 execve/CreateProcess 一个 Node 进程加载指定插件 index.js，
      通过预先打开的 fd 3（Unix）或 named pipe（Windows）与父 apollo 做 JSON-RPC bridge
...
CLI：apollo-sandbox --run-plugin \
       ...
       # 或（Windows）：
       --bridge-pipe \\.\pipe\ApolloBridge.<pid>.<uuid>
...
内部流程（跨平台一致的抽象）：
  ...
  4. spawn Node：argv = [node, <apollo-runtime>/plugin-host.mjs, <entry>, <dataDir>]
...
**核心抽象**：`SandboxProfile` 是 Apollo 内部的**能力级**中间表示，每平台 Backend 负责翻译为原生策略。**不假装存在跨平台 profile 格式**（详见 [SANDBOX-COMPAT-r1](./SANDBOX-COMPAT-r1.md) §S3 ADR-5）。

```rust
// crates/apollo-sandbox/src/backend/mod.rs
pub trait Backend {
...
Layer 1  bundled bwrap 二进制      — 编译嵌入，运行时 SHA256 校验后 execv（非 shell out）
         ...
         └─ protected subpath 重挂只读（.git / resolved gitdir / .apollo）
...
**Windows IPC**：无 fd 3 语义 → 用 named pipe（`\\.\pipe\ApolloBridge.<pid>.<uuid>`）+ `STARTUPINFOEXW` 显式继承 handle；协议层（JSON-RPC over NDJSON）跨平台一致。
...
**AppContainer ACE 污染防护**：AppliedHandle::drop() 里 revoke；启动时扫描并清理 orphaned ACE（前次崩溃遗留）；命名 `ApolloCode.Sandbox.<uuid>` 每 profile 独立。
...
**tier 冻结**：native-bridge 启动时探测一次，session 内不再变化；tier 变化必须重启 apollo。
...
**Windows mmap 注意**：ripgrep 底层 `grep-searcher` 在 Windows 下 mmap 会独占锁大文件；与 §4 Write/Edit 冲突时 apollo-search 自动降级 read+buffer 模式（不 mmap）。

#### 5.6.1 Worker 生命周期

```
启动：native-bridge 启动时 spawn apollo-search worker（一次）
...
复用 `apollo-sandbox --run-plugin` 已验证的 **fd 3（Unix）/ named pipe（Windows）+ NDJSON JSON-RPC 2.0** 通道模式（见 §5.3.2 / §6.4.3），仅 method 命名空间不同：
...
- 大文件读取用 mmap + `encoding_rs`（避免整文件读 UTF-8 再截）
  ...
  - Windows 下 mmap 独占锁行为已知；apollo-fs 需与 apollo-search 协调（读写不同文件时无冲突）
...
**BPE 表打包**：tiktoken 的 BPE 数据放主 npm 包 `@apollo-code/native-fs-common`（~2MB，跨平台共享），**worker 启动时**从该包路径加载到内存（非 addon 进程内静态链接）。native 平台包只装 worker 二进制，避免 8 target × 2MB 的冗余。
...
│  ├─ sandbox.ts              # 封装 apollo-sandbox 子进程调用 + probe/tier 缓存
│  ├─ search.ts               # 封装 apollo-search worker（IPC RPC 客户端）
│  ├─ fs.ts                   # 封装 apollo-fs worker（IPC RPC 客户端）
...
resolveBinary(kind):  # kind = 'sandbox' | 'search' | 'fs'
  ...
  try:
    ...
    binPath = readPackageJson(pkgDir).bin.apollo   # 平台包 bin 字段指向二进制
...
- `apollo doctor` sandbox 段显示当前 tier + search/fs worker 状态（pid / rss / restart_count）
...
**额外**：
- `@apollo-code/native-fs-common`（跨平台 BPE 表，主 npm 包，非平台包，fs worker 启动时读）
- `@apollo-code/native-sandbox-darwin-universal`（可选：universal2 合并包，Homebrew 用）
- ★ **r9 新增**：`@apollo-code/native-search-darwin-universal` / `@apollo-code/native-fs-darwin-universal`（可选 universal2 合并包）。原 napi-rs 不支持 universal `.node` 的限制（SANDBOX-COMPAT §S2 历史约束）**随二进制化解除**，darwin-arm64 + darwin-x64 可像 sandbox 一样 `lipo -create` 合并（Homebrew / 体积优化用）。
...
| **bundled bwrap 二进制 SHA256 校验**（运行时 verify_digest + CI 构建 digest 比对）   | apollo-sandbox 启动路径 + CI step                |
| Windows 二进制**必须** authenticode 签名（L1 起）；macOS **必须** notarize（L2 起）   | release CI 强制                                  |
| Named pipe / fd 3 IPC 句柄**禁止**继承给白名单外子进程                              | apollo-sandbox spawn 层控制                     |

### 5.11 里程碑

**每个里程碑发版前必须满足**（r9 调整：L1 砍平台范围，分层闸门）：
- **L1**：mac/linux **4-target** CI 全绿（含 native + escape test）+ `apollo doctor --strict` 4 target 各跑通过 + macOS notarize（L2 起）
...
- **L1（MVP）**：
  ...
  - `apollo-sandbox exec` + `--probe` 四挡（Full/Partial/Weak/None）
  - macOS Backend（codex seatbelt.rs）完整（T1/T2）
  - Linux Backend：bundled bwrap 默认 + landlock fallback（T3/T4 glibc）
  - `apollo-search` + `apollo-fs` 改**独立二进制 worker**（r9 架构变更：原 napi addon 作废，三产物形态统一，见 §5.6/§5.7）
...
- **L3**：
  ...
  - 外部安全审计发起
  - **剥离 codex-protocol → apollo-permission + codex-network-proxy**（L3 目标）

- **L4**：
  - `apollo-sandbox --test-escape` self red-team CLI
  - EV 代码签名（Windows）
  - 沙箱违规实时 dashboard（apollo doctor + 官网 troubleshooting）
  - apollo-search 支持自定义 codec / 二进制识别
...
| 维度 | 措施 |
|---|---|
| **upstream 跟踪** | 维护 `crates/apollo-sandbox-vendor/CODEOWNERS` + 每季度评估 rebase（记录 fork 偏离点 + 原因） |
...
| **fork 仓库镜像** | vendor 同时维护一份内部 mirror（防 upstream 强推删库 / force-push 历史）；mirror 地址记录在 `crates/apollo-sandbox-vendor/UPSTREAM.md` |
