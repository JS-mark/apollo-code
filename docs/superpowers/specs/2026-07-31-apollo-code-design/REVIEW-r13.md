...
| claude-code 能力 | Apollo spec 现状 | 判定 |
|---|---|---|
...
```
入口：apollo review [--base <ref>|--staged|--pr <n>|--range] + REPL /review + --severity-gate
...
```
定义文件（两层，项目覆盖全局同名）：
  ~/.apollo/agents/<name>.md  +  <cwd>/.apollo/agents/<name>.md
...
- 并发保护双闸：写入前 lockfile（<path>.apollolock，重试 3×1s，失败明确报错含 pid）
  + 成功后自动 backup（§8.6）
...
packages/testkit（dev-only）：
  ...
  3. tempApolloHome()：每测试隔离 HOME + 预填 config/credentials
...
- 新增 §16 → README 目录 + §10 里程碑 + §11.2 顶层命令（+`apollo review`）+ §11.4 slash（+/review）
