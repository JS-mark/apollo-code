...
> Apollo Code 当前处于 pre-1.0（L1–L4 阶段，见 §10）。在 1.0 GA 之前，仅"最新 0.x"享受安全补丁；老 0.x 尽力维护。1.0 发布后此表切换为标准的 major 支持策略。
...
| 要求 | 含义 |
|---|---|
| **每个契约必须有可验证的强制点** | CI / 单元测试 / ESLint / `apollo doctor --strict` / 集成测试之一；无强制点的描述性段落必须明确标「指导性，非强制」 |
