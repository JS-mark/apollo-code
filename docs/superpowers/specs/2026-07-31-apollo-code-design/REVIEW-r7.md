...
| **P0-6** `@include` 双白名单未防 symlink 逃逸完整闭环 | 🟢 **基本解决（残留 TOCTOU）** | §6.5.6 + AGENT.md §4.15："canonicalize 后判定，symlink 逃逸即拒绝" + 只吃 `.md` + 深度 8 + seen-set 环检测 + 单 compose 64 次上限。主攻面已堵。残留：spec 说"canonicalize 后判定"但未规定**原子 open + fstat**（仍是 stat-then-open 的时间窗）；`~/.apollo/` 白名单内未对敏感文件名加黑名单（虽 `.enc` 被 md-only 挡住，但理论上有 `.md` 命名的敏感残留）。窄窗口，可降级。 |
...
| **P1-3** Session JSONL Schema 版本 & 迁移未定义 | 🟡 **部分缓解** | §8.5.5 给**跨会话读**加了"版本号不兼容 → 降级只读 user 文本 + 标注部分跳过"；但 **JSONL 行级**仍无 `v: 1` 字段、无 `apollo resume` 跨版本迁移策略（仅 §8.2 W10 "校验版本号，不匹配拒绝"，未定义版本字段长什么样）。schema 演进无 versioning 仍是债。 |
| **P1-4** 同一文件的并发写（多 apollo 实例）无 file lock | 🟠 **仍开放** | §4 / §8.6 backups 仍无 `flock`；backup 目录无 `<session-id>` 隔离；`~/.apollo/state.lock` 保护 GC 未定义。两个 apollo 实例同 cwd 仍会 backup 互覆 / diff 冲突。 |
| **P1-5** Encrypted-file credentials 无 passphrase brute-force 防护 | 🟠 **仍开放** | §8.4 仅说"AES-256-GCM"，未指定 KDF（应 Argon2id）；§8.4.1 `auth.encfile.unlock_result` 记录 attempts 但**无冷却延迟 / 锁定策略**；离线爆破 `~/.apollo/credentials.enc` 无防护。 |
...
- **位置**：§8.3 config 分层（第 3 层 `<cwd>/.apollo/config.toml`）+ §11.3.9 `mcp.toml` + §14.4 project trust
- **问题**：`cd malicious-repo && apollo` 时，仓库自带的 `.apollo/config.toml` **自动加载、无信任门**。攻击者可在仓库里塞：
  ```toml
...
- **修复方向**：
  1. 项目级 config（`<cwd>/.apollo/config.toml`）首次加载必须**信任门**：弹窗展示将覆盖的 key 列表 + 来源仓库，用户确认后才生效（类似 VSCode workspace trust / claude-code 项目设置确认）
  2. **provider endpoint / telemetry sink / router** 这类"影响数据流向"的 key，项目级 config **禁止覆盖**（只能用户级或 env 设）；或覆盖时强制红条 + 显式 `--trust-project-config`
  3. 同理 `<cwd>/.apollo/mcp.toml` 自动连 MCP server 也要信任门（MCP server 注册的 tool 可批量申请权限，见 P1-2）
- **严重度理由**：需要克隆恶意仓库（非远程利用），但后果是**凭据外泄**，且 apollo 的核心用户场景就是 `cd` 进任意仓库跑——所以定为 P0。
...
#### NEW-P1-C · Memory 系统：模型面工具缺失（memory-guide 教模型用 `apollo.memory.*`，但那是插件 bridge）

- **位置**：§6.12 开篇"模型通过工具主动召回" + §6.12.3 memory-guide（priority 950，注入**模型** system prompt）写 "Use `apollo.memory.write(...)`" / "`apollo.memory.recall(query)`" + §6.12.5 MemoryBridge（属于**插件** `apollo` 对象）+ §4.3 内置工具清单（无任何 Memory 工具）
- **问题**：memory-guide 是注入**模型** system prompt 的 fragment（priority 950），教模型"用 `apollo.memory.write/recall`"。但 `apollo.memory.*` 是**插件 bridge** API（`activate(apollo)` 拿到的 `apollo` 对象），**模型（LLM）没有 `apollo` 对象**——模型只能调 ToolRegistry 里注册的工具（Read/Write/Bash...）。§4.3 工具清单和 §6.12 全文都**没有定义模型面的 Memory 工具**（如 `Memory.recall` / `Memory.write` / `Memory.read`），也没给它们的 inputSchema / permissionSpec / 截断规则。结果：memory-guide 在教模型调一个模型根本调不到的 API；pinned memory 注入有效，但**主动 recall/write 路径对模型是断的**。
- **修复方向**（二选一）：
  1. **推荐**：memory-runtime 在启动时向 ToolRegistry 注册模型面工具（`Memory.recall` / `Memory.read` / `Memory.write` / `Memory.update` / `Memory.delete` / `Memory.list`），复用 memory-runtime 内部逻辑；memory-guide 里的 `apollo.memory.*` 改成工具名（如 `Memory.recall`）；§4.3 工具清单补这一组（readonly 标注：recall/read/list 只读，write/update/delete 走 permission）。
...
- AGENT.md §4.13 第 243 行："`~/.apollo/logs/*.jsonl`"
- spec §8.1 存储树 + §8.4.1 + §8.7：`~/.apollo/telemetry/*.jsonl`（`apollo-YYYY-MM-DD.log` + `metrics-YYYY-MM-DD.jsonl`）
- **修复**：AGENT.md §4.13 改为 `~/.apollo/telemetry/*.jsonl` 对齐 spec（spec 是真源）。
...
- §6.10 第 395 行："插件系统 L1 依赖 §5.9 的 L2（apollo-sandbox --run-plugin 落地）"
...
- **NEW-P3-2** · `apollo history` 命令族操作的是 **sessions**（`~/.apollo/sessions/*.jsonl`），而 `~/.apollo/history`（§7.5.1 / §8.1）是**输入行历史**（↑↓ 翻历史）。两个"history"概念共用一个词，`apollo history list` 列的是 session 不是输入行。建议 §11.3.4 加一句澄清，或把输入行历史改名（如 `input-history`）。
...
| NEW-P2-C telemetry 路径不一致 | ✅ | AGENT.md §4.13 改 `~/.apollo/telemetry/*.jsonl` |
