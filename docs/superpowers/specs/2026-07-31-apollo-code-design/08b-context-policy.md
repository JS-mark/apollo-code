...
interface SemanticPolicyConfig {
  ...
  index_path: string                                // ~/.apollo/sessions/<id>.embeddings
  topk: number                                      // 默认 10
...
**权限**：`manifest.permissions.apollo` 必须含 `context.contribute` 才允许注册。policy 实现经沙箱执行（与 plugin tool 一样），其内部对 messages 的访问经 `apollo.session.getMessages()` RPC（只读快照）。
...
| **`apollo context keep` 的 pinned-to-context 标记必须可清除**（`unkeep`），不可永久锁住消息 | context 单元测试 |
| **手动 `apollo context compact` 必须尊重 preCompact hook veto**（hook 否决则不压 + 报错，不强压） | context + hooks 集成测试 |
...
| `context.keep_added` | 用户 `apollo context keep` 加 pinned-to-context 标记（r10，`{ target: 'message'\|'turn', id }`） |
...
- **L2**：`SummaryPolicy`（含失败回退）+ summary system prompt（untrusted 安全）+ 插件 `apollo.context.contributePolicy` 扩展点
...
**CLI 命令族**（`apollo context ...`，详见 §11.3）：

| 命令 | 作用 |
|---|---|
| `apollo context show [--json]` | 当前 token 占用 / 各来源占比（system / skill / memory:pinned / messages）/ 距下次压缩剩余预算 / 当前策略名 + 参数 |
| `apollo context diff` | 上次压缩移除了哪些消息（messageId + 摘要首行 + turnId），用户可核对是否误删重要信息 |
| `apollo context keep <messageId \| turnId>` | 给消息/turn 打 **pinned-to-context** 标记 → 压缩时**强制保留**（独立于 Memory 的 pinned，这是 context 级保留，session 结束失效） |
| `apollo context unkeep <messageId \| turnId>` | 清除 pinned-to-context 标记 |
| `apollo context compact [--strategy sliding\|summary]` | 手动触发压缩（可指定策略，否则用当前策略）；尊重 preCompact hook veto（hook 返回 veto 则不压 + 报错） |
| `apollo context policy <get\|set>` | 查/改当前 ContextPolicy（`set` 走配置校验，立即生效下一轮） |
...
- 选中某条消息按 K → 等价 `apollo context keep`
- 按 C → 手动压缩（二次确认）
...
**与 §8.2b JSONL 分段的关系**：`apollo context diff` 读的是 SessionState 的 messages 与上次压缩前的快照对比；大 session 走分段加载（§8.2b）只读最近窗口，diff 也只覆盖窗口内。
...
**接入方式**：
- 参数当前值优先从 `~/.apollo/tuning/context.jsonl` 读（§15.2 Layer B）；未调过则用 `config.toml [context]` 值；config 未设则用内置默认。
