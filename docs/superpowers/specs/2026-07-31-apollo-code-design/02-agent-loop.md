...
- **B5 并行语义**（明确）：
  ...
  - ★ **r9 新增：框架级 `ctx.kv` 命名空间 store**——hook handler 若需跨调用共享状态，**优先用 `ctx.kv`**（框架保证命名空间级互斥）而非自管全局可变状态。`ctx.kv` 按 (event + 来源 plugin/toolUseId) 命名空间隔离：同一 tool_use 的 pipeline 内 handler 可见共享；不同 tool_use 之间 kv 不共享（避免 parallelInvoke 竞态）。API 见 §6.4.1 `apollo.hook.kv`。作者若仍坚持自管全局状态，须自行加锁（框架不兜底）。
