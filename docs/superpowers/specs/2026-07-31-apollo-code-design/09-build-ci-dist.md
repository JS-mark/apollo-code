...
jobs:
  ...
  native:
    ...
    steps:
      ...
      - # Linux target: 校验 bundled bwrap 二进制 SHA256 与 BWRAP_DIGEST.txt 一致
        ...
        run: apollo-sandbox --verify-bwrap-digest
  ...
  sandbox-escape:
    ...
    steps:
      - 加载 apollo-sandbox 产物 + 平台探针（codex get_platform_sandbox: seatbelt/bwrap+seccomp/landlock ABI/Job 版本）
  ...
  release:
    ...
    steps:
      - changeset publish   → npm + GitHub Release（L1: 12 平台包 = 4 target × 3 crate；L2: 24 平台包 = 8 target × 3 crate；+ apollo-code + fs-common）
...
| `apollo-code.pkg` / `apollo-code.dmg` **必须** notarize（L2+）                        | release CI                                       |
