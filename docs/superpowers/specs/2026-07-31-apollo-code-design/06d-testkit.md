...
| 3 | `tempApolloHome()` | 隔离环境 | config / credentials / memory 测试 |
...
import { MockProvider, scriptChunks } from '@apollo-code/testkit'
...
// tempApolloHome()：每测试隔离 HOME + 预填结构
const home = await tempApolloHome({ config: { [memory]: { enabled: true } }, credentials: 'fake' })
...
`packages/testkit` 加入 §1 workspace 包清单（dev-only，不进 §9 发布产物）。发布名 `@apollo-code/testkit` **私有**（不发包，workspace 内引用），插件/hook 作者的测试可引用。

### 6.13.6 里程碑

- **L1**：首批设施（MockProvider + fakeClock + tempApolloHome）**与 core 同 PR 交付（测试先行）**——spec 的 L1 强制点测试直接建立在其上。
...
| tempApolloHome 必须真实隔离 HOME（不污染开发者机器） | 测试 teardown 断言 |
