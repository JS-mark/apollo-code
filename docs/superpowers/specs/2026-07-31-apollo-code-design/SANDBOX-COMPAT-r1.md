...
# Apollo Code · Rust 沙箱跨平台兼容性白皮书 (r1)

> **状态**：Approved（2026-08-01）
> **文档类型**：架构决策记录 (ADR) + 兼容性规约（Compatibility Spec）
> **范围**：`crates/apollo-sandbox` + `packages/native-bridge` 的沙箱能力
...
4. **零系统工具依赖**（r3 修订）：用户 `npm i -g apollo-code` 后**不需要额外装任何系统工具**。沙箱所需的一切（包括 Linux 上的 bwrap）**都随 apollo-code 自带**：bundled bwrap 二进制在编译时嵌入、运行时 SHA256 校验后执行（fork 自 codex 的机制）。这不是"shell out 到系统 bwrap"——用户无需 `apt install bubblewrap`。
5. **一等公民**：沙箱是 Apollo Code 的核心卖点，与 §4 权限体系正交但同等重要；**L1 聚焦 mac/linux 4 target 先把核心 loop 跑通，Windows/musl L2 补齐**（r9 调整：非"某平台永久 no-sandbox"，而是分里程碑交付）。
...
**每 target 3 个 native crate**（`apollo-sandbox` / `apollo-search` / `apollo-fs`，r9 后均为独立二进制）→ **L1: 4 target × 3 = 12 个平台包；L2: 8 target × 3 = 24 个 `@apollo-code/native-*` 平台包**，通过 `native-bridge` 的 `optionalDependencies` 挂载。
...
**决策**：Apollo Code **整套 fork OpenAI [codex](https://github.com/openai/codex) monorepo 里的三个沙箱 crate** 作为底座，vendor 进 `crates/apollo-sandbox/vendor/codex/`，保留 Apache-2.0 + NOTICE 归属，逐步剥离 codex-* workspace 依赖。
...
2. **workspace 依赖处理：全 vendor 后逐步剥离**
   - L1：把上述 12 个 codex-* crate 连同三个沙箱 crate 一起 vendor，改 workspace 名为 `apollo-sandbox-vendor/`，先跑通
   - L2-L3：逐步把 `codex-protocol` → `apollo-protocol`、`codex-utils-*` → `apollo-utils-*` 重写替换，最终消除 codex 包名残留
   - L4：只剩沙箱三件套保留 codex attribution，其余完全 apollo 化
...
- **License**：codex Apache-2.0，Apollo Code Apache-2.0，双向兼容；`NOTICE` 文件列 OpenAI/Apollo Code 联合版权 + codex crate 归属
...
Apollo 内部对每 target 声明其当前**沙箱等级**，由 `native-bridge` 启动时探测得出：
...
- **UI 层必须能显示当前 tier**（`apollo doctor` + 状态栏徽章）。

### ADR-5 · Profile 抽象：能力归一，机制分背端

**决策**：`SandboxProfile` 是 Apollo 内部**能力级**中间表示，向下由每 target 的 `Backend` trait 分别翻译成 sbpl / landlock rules / AppContainer SID+ACL；**不假装存在跨平台 profile 格式**。

```rust
// crates/apollo-sandbox/src/profile.rs
...
`crates/apollo-sandbox/src/backend/macos/sbpl.rs` 独立子模块：
...
| MAC-R5 | Universal2 打包（合并 T1+T2） | 用 `lipo -create` 打进单一 `apollo-sandbox` bin；Homebrew formula 依赖 universal2 |

### S4.5 Universal2 分发

- CI 分别在 `macos-14` (arm64) / `macos-13` (x64) 构建 `apollo-sandbox`；额外 `lipo -create` 步骤合并为 universal2 二进制，随 `@apollo-code/native-sandbox-darwin-universal` 包发布。
- napi-rs 产物 `apollo-search.node` / `apollo-fs.node` 保持双 target 独立包（napi-rs 不支持 universal .node）。
...
`crates/apollo-sandbox/vendor/codex/linux-sandbox/` 内：
...
- Alpine 用户 `apk add apollo-code`（v2）时提供 musl binary；npm 用户 pnpm 按 `libc` 字段挑选

### S5.6 Linux 特有风险（r3 更新）

| ID | 风险 | 缓解 |
|---|---|---|
| LIN-R1 | RHEL 8 / Amazon Linux 2 落 Weak 挡（userns 禁 + landlock <5.13） | UI 首屏 + `apollo doctor` 明示；不阻塞使用 |
...
| LIN-R4 | seccomp filter 永久无法撤销，需 spawn 子进程 | apollo-sandbox 本就独立 bin + bwrap 子进程，天然满足 |
...
```rust
// crates/apollo-sandbox/src/backend/windows/tier1.rs
...
- **AppContainer 命名规则**：`ApolloCode.Sandbox.<uuid>`，uuid 每个 profile 实例独立，避免命名冲突
...
- 无 fd 3 语义 → 用 `CreateNamedPipeW(\\.\pipe\ApolloBridge.<pid>.<uuid>, ...)` 建 named pipe
- 派生子进程时用 `STARTUPINFOEXW` + `PROC_THREAD_ATTRIBUTE_HANDLE_LIST` 显式继承 pipe handle
- 子进程内 plugin-host.mjs 通过 `net.Socket({ fd })` 兜底不通用；改为约定环境变量 `APOLLO_BRIDGE_PIPE=\\.\pipe\...` → Node 侧 `net.createConnection({ path: process.env.APOLLO_BRIDGE_PIPE })`
...
| WIN-R3 | Windows Defender / SmartScreen 屏蔽未签名 apollo-sandbox.exe | L1 起做 authenticode 签名；L4 前升级 EV 证书 |
...
native-bridge init 顺序：
  ...
  2. spawn `apollo-sandbox --probe` (5s 超时)
...
**tier 冻结规则**：一旦 session 内确定 tier，中途不再重新探测；改变需重启 apollo。

---

## §S8 逃逸测试策略（CI 硬要求）

`crates/apollo-sandbox/tests/escape/`：
...
以下平台 apollo `--probe` 后直接返回 tier=None，`apollo doctor` 说明"当前系统沙箱能力不可用"：
...
| **L4** | 沙箱违规实时 telemetry；`apollo-sandbox --test-escape` self red-team CLI；EV 代码签名 | 所有 target Full |
...
2. `apollo doctor --strict` 在对应 target 各跑一次通过（L1 跑 4，L2+ 跑 8）
...
| L3（seccomp 无法撤销） | §S5.6 LIN-R2 (apollo-sandbox 独立 bin 天然满足) | ✅ |
...
1. `crates/apollo-sandbox/` 骨架 + **vendor codex 三件套**：`vendor/codex/{sandboxing,linux-sandbox,windows-sandbox-rs}/` + 保留 LICENSE/NOTICE（OpenAI 归属）
2. **vendor codex workspace 依赖**（ADR-1）：`vendor/codex/{protocol,network-proxy,execpolicy,process-hardening,install-context,utils-*,otel}/` 共 12 个 crate，改 workspace 名为 `apollo-sandbox-vendor`，先跑通编译
...
5. **workspace 依赖剥离计划**（L1-L4 渐进）：
   ...
   - L2：剥离 `codex-utils-*`（路径/字符串/pty 工具类）→ 合并进 `apollo-utils`
   - L3：剥离 `codex-protocol`（PermissionProfile/SandboxPolicy）→ 重写为 `apollo-permission`；剥离 `codex-network-proxy`
   - L4：剥离 `codex-execpolicy` / `codex-process-hardening`；最终仅沙箱三件套保留 codex attribution
6. **bundled bwrap 供应链**：保留 codex 的 `verify_digest`（SHA256）机制；CI 每次构建校验 bwrap 二进制 digest；签名后分发；记录 expected digest 到 `crates/apollo-sandbox/BWRAP_DIGEST.txt`
...
10. `apollo doctor` 增加 sandbox 段（含 codex crate 版本 + bwrap digest 回显）
