...
| 7 | **hook 框架级 KV store** | [`02-agent-loop.md`](./02-agent-loop.md) §2.5 B5（ctx.kv 命名空间 + 框架保证互斥）+ §2.6 执行语义（命名空间隔离规则）；[`06a-plugins-core.md`](./06a-plugins-core.md) §6.4.1 ApolloBridge 新增 `apollo.hook.kv`（get/set/delete/clear） | ✅ |
