...
- §4 Monorepo 边界规则（**强约束**）
  ...
  - §4.15 `@include` 机制硬约束（仅 apollo prompt 管线，双白名单，只 md，深度 8）
- §5 代码规范
- §6 测试规范
- §7 提交规范
- §8 目录新增流程
...
1. **Rust 沙箱是一等公民** —— 项目的**安全基座**。所有执行第三方 / 用户 / 未审计代码的路径（Bash 工具、Plugin 宿主）**默认**走 `apollo-sandbox` 独立进程 + syscall 隔离；TS 侧不允许自建 sandbox。绕过 = 破坏信任模型（详见 AGENT.md §4.7 + spec §5.3 / §6.4.3）。
...
- 搜索优先用本仓库自带的 `apollo-search`（Rust ripgrep 绑定），而不是 Grep/Glob。
...
- 修改 Provider 抽象接口
- 修改权限模型
- **触及 Rust 沙箱边界**（`apollo-sandbox` CLI 面、profile 生成、`--run-plugin` 协议）
...
- **禁止** 在 `apollo` JSBridge 之外给插件开旁路 API（不允许暴露 `native` / `core` / `runner` / 其它插件对象）；**例外**：`apollo.provider.register` 受控开放（仅 `kind:'provider'` 插件注册 ProviderClient，必经 Router，见 spec PLUGIN-PROVIDER-r1）；provider **直调**入口（stream/complete/getCredential）仍禁止
- **禁止** 在 `plugin-runtime` 用 `node:vm` / `worker_threads` 跑插件，或内部直接 `child_process.spawn(node)`。插件宿主**必须**走 `native-bridge.runPlugin()` → `apollo-sandbox --run-plugin` 独立子进程（Rust 沙箱一等公民规则，见 AGENT.md §4.7）
...
- **禁止** 在 provider 实现里绕过 verify-first-store-second（`apollo login` 必须先调最小验证接口，2xx 才落盘）
...
- **禁止** 绕过 `packages/memory-runtime` 直接读写 `~/.apollo/memory/` 或 `<cwd>/.apollo/memory/`（`storage` / `tools` / provider 均不允许，见 AGENT.md §4.14）
...
- **禁止** 在 `provider-*` / `router` / `tools` 里另起 memory 提示词（只允许 `builtin:memory-guide` + `apollo.memory.contributePrompt`）
- **禁止** 未声明 `manifest.permissions.memory` 就调用 `apollo.memory.*`（bridge 直接 `-32601`）
- **禁止** 在 `packages/core` 之外实现 `@include` 展开逻辑（`prompt-loader.ts` 是唯一实现）
- **禁止** `@include` 引入 `.md` 以外的扩展名（json / toml / 代码文件一律拒绝）
- **禁止** 在工具输出 / 模型消息 / 用户对话文本里做 `@include` 展开（仅限 apollo 内部 prompt 装载管线）
- **禁止** `@include` 路径落在 `<cwd>/**` 或 `~/.apollo/**` 之外（canonicalize 后判定，symlink 逃逸即拒绝）
